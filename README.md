...
Filenames carry their dimensions (`header_capsule_920x430.png`), since Steamworks has two 920×430
fields and four assets whose names all start with "capsule".

Two of the nine assets Steamworks asks for are missing from that table, and the tool names them at
the end of every run: screenshots have to be actual gameplay, and the library logo is drawn on
...
is required; the defaults centre every rectangle and take as much of the picture as its shape allows.
...
above, stops the run. A misspelling there still produces every capsule, framed exactly as if nothing
had been asked for.
...
about it. The filter comes from the numbers rather than from config: an exact whole-number
enlargement is nearest neighbour, since a game render scaled 2× by anything that interpolates comes
back as mush; shrinking is a box filter, since dropping rows throws away the thin bright lines that
are the whole subject of a vector render; anything else is bilinear.

`library_hero` is 3840 wide and almost no source is, so an enlargement is called out in capitals
rather than left to be noticed on the store page. The run also prints where Steam's 860×380 safe zone
(the part of the hero that survives at every client width) lands back in the source, so what will
never be cropped away can be checked in the plate.
